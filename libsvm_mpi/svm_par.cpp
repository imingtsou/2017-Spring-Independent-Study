...
#include "ThreadPool/DistributedCache.h"
#include "mpi.h"
#include <iostream>
...
int Solver::select_working_set(int &out_i, int &out_j)
{
	...
	auto func2 = [&] (int tid) {
			...
			if(y[j]==+1)
			{
				if (!is_lower_bound(j))
				{
					...
					if (grad_diff > 0)
					{
						...
						if (obj_diff <= t_obj_diff_min)
						{
							t_Gmin_idx=j;
							t_obj_diff_min = obj_diff;
						}
					}
				}
			}
...

		{
			...
			if (t_obj_diff_min < obj_diff_min) {
				obj_diff_min = t_obj_diff_min;
				Gmin_idx = t_Gmin_idx;
			} 
		}
	};
...
public:
	...
	:Kernel(prob.l, prob.x, param)
	{
		...
		if (param.shrinking) {
			...
			if (param.MPI_flag) {
				cache = new DistributedCache(prob.l, y, [this](int i, int j) -> double {
...
static void solve_c_svc(
	...
	double *alpha, Solver::SolutionInfo* si, double Cp, double Cn, int my_rank)
{
	int l = prob->l;
	schar *y = new schar[l];
	...
	SVC_Q Q(*prob, *param, y); // Set up SVC cache
	if (my_rank == 0) {
...
static decision_function svm_train_one(
	const svm_problem *prob, const svm_parameter *param,
	double Cp, double Cn, int my_rank)
{
	...
	{
		case C_SVC:
			solve_c_svc(prob,param,alpha,&si,Cp,Cn,my_rank);
			break;
		case NU_SVC:
...
svm_model *svm_train(const svm_problem *prob, const svm_parameter *param)
{
	   ...
	   param->svm_type == NU_SVR)
	{
		...
		decision_function f = svm_train_one(prob,param,0,0,my_rank);
	...
	else
	{
		...
		for(i=0;i<nr_class;i++)
		{
			for(int j=i+1;j<nr_class;j++)
			{
				...
				f[p] = svm_train_one(&sub_prob,param,weighted_C[i],weighted_C[j],my_rank);
				for(k=0;k<ci;k++)
