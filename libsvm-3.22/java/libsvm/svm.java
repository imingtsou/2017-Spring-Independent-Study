...
class SVC_Q extends Kernel
{
	...
	float[] get_Q(int i, int len)
	{
		...
		{
			// omp parallel for private(j) schedule(dynamic) threadNum(64)
			for (int jj = start; jj < len; jj++) {
				data[0][jj] = (float)(y[i] * y[jj] * kernel_function(i, jj));
			}
		}
		return data[0];
	}
...
public class svm {
	...
	public static double svm_predict_values(svm_model model, svm_node[] x, double[] dec_values)
	{
		...
		else
		{
			...
			// omp parallel for private(i) schedule(dynamic) threadNum(64)
			for (int ii = 0; ii < l; ii++) {
				kvalue[ii] = Kernel.k_function(x,model.SV[ii], model.param);
			}
